...
- **slug 保留中文/空格**：slug = 文件名原样（无编码），URL 端用 `encodeURIComponent` 包装。文件路径 `<slug>.md` ↔ URL `/posts/<encodeURIComponent(slug)>/`。`getPostBySlug` 同时兼容 raw 和 encoded slug（因为 `<Link>` pre-render 用 encoded 形式、`generateStaticParams` 用 raw 形式）。
